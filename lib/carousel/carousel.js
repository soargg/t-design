...
import { defaultScrollInterpolator, stackScrollInterpolator, tinderScrollInterpolator, defaultAnimatedStyles, shiftAnimatedStyles, stackAnimatedStyles, tinderAnimatedStyles } from './animations';
...
export default class Carousel extends Component {
    constructor(props) {
        super(props);
        this._carouselRef = null; // ScrollView 实例引用
        this._scrollOffsetRef = 0;
        this._scrollStartOffset = 0; // 拖拽开始时的位置 计算delta，求出下一个索引
        this._scrollEndOffset = 0; // 结束拖拽是 ScrollView的滚动量，用来和 _currentContentOffset作比较
        this._scrollEnabled = this.props.scrollEnabled !== false; // 视图可滚动
        this._positions = []; // 记录每个元素的其实位置
        this._currentContentOffset = 0; // ScrollView滚动量
        this._mounted = false; // 已挂载
        this._onLayoutInitDone = false; // 绘制完成
        // autoplay
        this._autoplay = false; // 
        this._autoplaying = false; // 自动切换中。。。
        // timeout
    ...
    _setScrollHandler(props) {
        ...
        try {
            ...
            if (props.onScroll && Array.isArray(props.onScroll._argMapping)) {
                ...
                const argMapping = (props.onScroll._argMapping);
                argMapping.pop();
    ...
    // ScrollView的实例引用
    _getWrappedRef() {
        if (this._carouselRef && ((this._needsScrollView() && this._carouselRef.scrollTo) ||
    ...
    _initPositionsAndInterpolators(props = this.props) {
        ...
        const interpolators = [];
        this._positions = [];
        this._getCustomData(props).forEach((_, index) => {
    ...
    _onStartShouldSetResponderCapture(event, gestureState) {
    ...
    _renderItem({ item, index }) {
        const { interpolators } = this.state;
        const { hasParallaxImages, itemWidth, itemHeight, keyExtractor, renderItem, sliderHeight, sliderWidth, slideStyle, vertical } = this.props;
        const animatedValue = interpolators && interpolators[index];
        if (!animatedValue) {
            return null;
        }
        ...
        return (<Component style={[mainDimension, slideStyle, animatedStyle]} pointerEvents={'box-none'} {...specificProps}>
                {renderItem(item, index, parallaxProps)}
            </Component>);
    }
    _getComponentOverridableProps() {
        const { enableMomentum = false, itemWidth, itemHeight, loopClonesPerSide, sliderWidth, sliderHeight, vertical } = this.props;
    ...
    _getComponentStaticProps() {
        ...
        const { containerCustomStyle = {}, contentContainerCustomStyle, keyExtractor, sliderWidth, sliderHeight, vertical = false } = this.props;
