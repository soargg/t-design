...
export class Swiper extends React.PureComponent {
    constructor(props) {
        super(props);
        this.initialRender = true; // 组件初始化
        this.internals = {
            isScrolling: false,
            offset: { x: 0, y: 0 },
        };
    ...
    _scrollByIndexStep(step) {
        if (this.internals.isScrolling || this.state.total < 2)
            return;
        this.scrollTo(this.state.index + step, true);
    }
    getNextState(props, state = initialState, updateIndex = false) {
        // offset是一个对象，这样些取消引用
        const nextState = { ...state, offset: { x: 0, y: 0 } };
    ...
    _loopJump() {
        ...
        requestAnimationFrame(() => {
            if (this.refScrollView) {
                if (this.refScrollView.setPageWithoutAnimation) {
                    let index = this.state.index + (this.props.loop ? 1 : 0);
                    ;
                    this.refScrollView.setPageWithoutAnimation(index);
                }
                else {
                    const offset = this.internals.offset;
                    this.refScrollView.scrollTo({ ...offset, animated: false });
    ...
    renderIndicator() {
        ...
        return (<View pointerEvents="none" style={styles.indicatorWrapper}>
                <View style={[styles.indicator, dotWrapStyle]}>
                    <Dot activeDotColor="#ffffff" containerWidth={50} curPage={index} maxPage={total}/>
                </View>
            </View>);
    }
    getScrollViewProps() {
        // 初始化一些ScrollView的Props
        const { horizontal = true, showsHorizontalScrollIndicator = false, showsVerticalScrollIndicator = false, bounces = false, scrollsToTop = false, automaticallyAdjustContentInsets = false, removeClippedSubviews = true, } = this.props;
        return {
            horizontal,
    ...
    renderSwiperItems() {
        ...
        if (total > 0) {
            ...
            return nodeList.map((swiperItem, idx) => {
                if (loadMinimal) {
                    if ((idx >= index + loopVal - loadMinimalSize &&
                        idx <= index + loopVal + loadMinimalSize) ||
                        // The real first swiper should be keep
                        (loop && idx === 1) ||
                        // The real last swiper should be keep
                        (loop && idx === total - 1)) {
                        return (<View key={idx} style={itemStyle}>
                                {swiperItem}
                            </View>);
                    }
                    else {
                        return (<View style={loadingStyle} key={idx}>
    ...
    renderScrollView() {
        const { style, scrollViewStyle } = this.props;
        return (<ScrollView ref={scroller => { this.refScrollView = scroller; }} contentContainerStyle={[styles.wrapper, style]} style={scrollViewStyle} pagingEnabled {...this.getScrollViewProps()} contentOffset={this.state.offset} onScrollBeginDrag={this._onScrollBeginDrag.bind(this)} onScrollEndDrag={this._onScrollEndDrag.bind(this)} onMomentumScrollEnd={this._onScrollEnd.bind(this)}>
                {this.renderSwiperItems()}
            </ScrollView>);
    }
    render() {
        const { containerStyle, showsButtons = false, showDots = true } = this.props;
