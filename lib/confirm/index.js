...
class ConfirmView extends React.PureComponent {
    ...
    render() {
        const { children, title, okText = '我知道', cancelText = '取消', okStyle, cancelStyle, okBtnStyle, cancelBtnStyle, onCancelPress, onOkPress, onFinish } = this.props;
        return (<View style={styles.mask}>
                <AnimationView active={this.state.visible} animationType="fadeInOut" style={styles.card} onFinish={onFinish}>
            ...
            // 取消按钮
            onCancelPress ?
                <TouchableOpacity style={[styles.confirmButton, styles.division, cancelBtnStyle]} activeOpacity={0.8} onPress={() => {
                        this.setState({
                            visible: false
                        });
                        onCancelPress();
                ...
                :
                    null}
                        {/* 确认按钮 */}
                        <TouchableOpacity style={[styles.confirmButton, okBtnStyle]} activeOpacity={0.8} onPress={() => {
                // 消失动画
                this.setState({
                    visible: false
                });
...
ConfirmView.contextType = PortalContext;
/**
 * @method Confirm
...
export default function Confirm(content) {
    let title = '', btnText = ['确认', '取消'], cancel = true, zIndex = 99;
    let btnTxtStyles = [null, null];
    let btnStyles = [null, null];
    ...
    const contentEle = isFunction(content) ? content() : content;
    ...
    return new Promise(resolve => {
        ...
        const key = Portal.mount(<ConfirmView title={title} okText={okText} cancelText={cancelText} okStyle={okStyle} cancelStyle={cancelStyle} okBtnStyle={okBtnStyle} cancelBtnStyle={cancelBtnStyle} onOkPress={() => {
                handleResolve(true);
            }} onCancelPress={onCancelPress} onFinish={() => {
                // 卸载Comfirm
            }}>
                {contentEle}
            </ConfirmView>, zIndex);
...
const cardWidth = screenWidth * 542 / 750;
