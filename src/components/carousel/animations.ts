...
import { isAndroid as IS_ANDROID } from '../common';
...
//     (index + 1) * sizeRef // active - 1
// ]
export function getInputRangeFromIndexes(range: number[], index: number, carouselProps: CarouselProps) {
...
export function defaultScrollInterpolator (index: number, carouselProps: CarouselProps) {
    const range = [1, 0, -1];
...
export function defaultAnimatedStyles (animatedValue: Animated.AnimatedInterpolation, carouselProps: CarouselProps) {
    let animatedOpacity = {};
    let animatedScale = {};
...
export function shiftAnimatedStyles(animatedValue: Animated.AnimatedInterpolation, carouselProps: CarouselProps) {
    let animatedOpacity = {};
    let animatedScale = {};
...
export function stackScrollInterpolator (index: number, carouselProps: CarouselProps) {
...
export function stackAnimatedStyles (index: number, animatedValue: Animated.AnimatedInterpolation, carouselProps: CarouselProps, cardOffset: number) {
    const sizeRef = carouselProps.vertical ? carouselProps.itemHeight : carouselProps.itemWidth;
...
// This means that the item with the higher zIndex (and therefore the tap receiver) remains the one AFTER the currently active item
// The `elevation` property compensates for that only visually, which is not good enough
export function tinderScrollInterpolator(index: number, carouselProps: CarouselProps) {
    const range = IS_ANDROID ?
        [1, 0, -1, -2, -3] :
        [3, 2, 1, 0, -1];
    const inputRange = getInputRangeFromIndexes(range, index, carouselProps);
...
export function tinderAnimatedStyles (index: number, animatedValue: Animated.AnimatedInterpolation, carouselProps: CarouselProps, cardOffset: number) {
    const sizeRef = carouselProps.vertical ? carouselProps.itemHeight : carouselProps.itemWidth;
