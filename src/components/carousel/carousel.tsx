...
import { Animated, Easing, FlatList, I18nManager, ScrollView, View, ViewPropTypes } from 'react-native';
import { isiOS as IS_IOS } from '../common';
import { shallowCompare, isFunction } from '../common/utils';
import {
...
export const AnimatedFlatList = FlatList ? Animated.createAnimatedComponent(FlatList) : null;
const AnimatedScrollView = Animated.createAnimatedComponent(ScrollView);
const IS_RTL = I18nManager.isRTL;

export default class Carousel<ItemT = any> extends Component<CarouselProps<ItemT>, CarouselState> {
    static propTypes: any;
    static defaultProps: any;

    private _carouselRef: any = null; // ScrollView 实例引用
    private _activeItem: number; // 当前索引
    private _previousActiveItem: number;
    private _previousFirstItem: number;
    private _previousItemsLength: number;
    private _itemToSnapTo: number; // 驻停索引
    private _scrollOffsetRef: number = 0;

    private _scrollStartOffset = 0; // 拖拽开始时的位置 计算delta，求出下一个索引
    private _scrollStartActive: number;
    private _scrollEndOffset: number = 0; // 结束拖拽是 ScrollView的滚动量，用来和 _currentContentOffset作比较
    private _scrollEndActive: number; // 滚动结束时的索引

    private _scrollEnabled: boolean = this.props.scrollEnabled !== false; // 视图可滚动
    private _positions: Array<{ start: number, end: number }> = []; // 记录每个元素的其实位置
    private _currentContentOffset: number = 0; // ScrollView滚动量
    private _scrollPos: Animated.Value; // 伴随滚动的动画参数
    private _onScrollHandler: any;

    private _mounted: boolean = false; // 已挂载
    private _onLayoutInitDone: boolean = false; // 绘制完成

    // autoplay
    private _autoplay: boolean = false; // 
    private _autoplaying: boolean = false; // 自动切换中。。。

    // timeout
    private _apparitionTimeout: any = null;
    private _hackSlideAnimationTimeout: any = null;
    private _enableAutoplayTimeout: any = null;
    private _autoplayTimeout: any = null;
    private _snapNoMomentumTimeout: any = null;
    private _edgeItemTimeout: any = null;
    private _lockScrollTimeout: any = null;
    private _autoplayInterval: any = null;

    // used when momentum is enabled to prevent an issue with edges items
    private _onScrollTriggered: boolean = true;
    private _canFireBeforeCallback: boolean = false;
    private _canFireCallback: boolean = false;

    // used to work around a FlatList bug
    private _lastScrollDate: number = 0;
    // This bool aims at fixing an iOS bug due to scrollTo that triggers onMomentumScrollEnd.
    // onMomentumScrollEnd fires this._snapScroll, thus creating an infinite loop.
    private _ignoreNextMomentum: boolean = false;

    // 对外接口
    public get displayname() {
        return 'Carousel';
    }
    public get realIndex() {
    ...
    constructor(props: CarouselProps<ItemT>) {
        super(props);
        ...
        const initialActiveItem = this._getFirstItem(defaultIndex);
        ...
        // Warnings
        if (!ViewPropTypes) {
            console.warn('snap-carousel: It is recommended to use at least version 0.44 of React Native with the plugin');
        }
        if (!props.vertical && (!props.sliderWidth || !props.itemWidth)) {
            console.error('snap-carousel: You need to specify both `sliderWidth` and `itemWidth` for horizontal carousels');
        }
        if (props.vertical && (!props.sliderHeight || !props.itemHeight)) {
            console.error('snap-carousel: You need to specify both `sliderHeight` and `itemHeight` for vertical carousels');
        }
        if (props.apparitionDelay && !IS_IOS && !props.useScrollView) {
            console.warn('snap-carousel: Using `apparitionDelay` on Android is not recommended since it can lead to rendering issues');
        }
    }

    componentDidMount() {
        const { apparitionDelay, autoplay = false, defaultIndex=0 } = this.props;
        const _firstItem = this._getFirstItem(defaultIndex);
    ...
    shouldComponentUpdate(nextProps: CarouselProps<ItemT>, nextState: CarouselState): boolean {
    ...
    componentDidUpdate(prevProps: CarouselProps<ItemT>) {
        const { interpolators } = this.state;
        const { defaultIndex = 0, itemHeight, itemWidth, scrollEnabled, sliderHeight, sliderWidth } = this.props;
        ...
        const nextFirstItem = this._getFirstItem(defaultIndex, this.props);
    ...
    private _setScrollHandler(props: CarouselProps<ItemT>) {
        ...
        try {
            // props.onScroll 传入的是Animated.event; 这样要额外处理映射
            if (props.onScroll && Array.isArray((props.onScroll as any)._argMapping)) {
                // Because of a react-native issue https://github.com/facebook/react-native/issues/13294
                const argMapping = ((props.onScroll as any)._argMapping) as Array<any>;

                argMapping.pop();
                const [argMap] = argMapping;
    ...
    private _needsRTLAdaptations() {
        const { vertical } = this.props;
        return IS_RTL && !IS_IOS && !vertical;
    }

    private _canLockScroll() {
    ...
    private _shouldAnimateSlides(props = this.props) {
        const { layout='linear', inactiveSlideOpacity, inactiveSlideScale, scrollInterpolator, slideInterpolatedStyle } = props;
    ...
    private _shouldUseCustomAnimation() {
    ...
    private _shouldUseShiftLayout() {
        const { inactiveSlideShift, layout } = this.props;
        return layout === 'default' && inactiveSlideShift !== 0;
    }

    private _shouldUseStackLayout() {
        return this.props.layout === 'stack';
    }

    private _shouldUseTinderLayout() {
        return this.props.layout === 'tinder';
    }

    private _getCustomData(props = this.props) {
    ...
    private _getCustomDataLength(props = this.props) {
    ...
    private _getCustomIndex(index: number, props = this.props) {
    ...
    private _getDataIndex(index: number) {
    ...
    // 获取时期位置，循环是特殊处理
    private _getPositionIndex(index: number) {
        const { loop = false, loopClonesPerSide = 3 } = this.props;
        return loop ? index + loopClonesPerSide : index;
    }

    // 初始位置计算
    private _getFirstItem(index: number, props = this.props) {
     ...
     * @void
     */
    private _setScrollEnabled(scrollEnabled: boolean = true) {
    ...
    // 为每个Item设置唯一Key
    private _getKeyExtractor(_: ItemT, index: number) {
        return this._needsScrollView() ? `scrollview-item-${index}` : `flatlist-item-${index}`;
    }

    // 获取滚动偏移量
    private _getScrollOffset(event: NativeSyntheticEvent<NativeScrollEvent>): number {
    ...
    private _getContainerInnerMargin(opposite = false) {
    ...
    private _getActiveItem(offset: number): number {
        const { swipeThreshold = 20 } = this.props;
    ...
    private _initPositionsAndInterpolators(props = this.props) {
        ...
        const interpolators: Array<Animated.Value | Animated.AnimatedInterpolation> = [];
        
        this._positions = [];

        this._getCustomData(props).forEach((_, index: number) => {
            ...
            if (!this._shouldAnimateSlides(props)) {
                ...
                let interpolator: Animated.InterpolationConfigType;
                // 先创建动画映射
    ...
    private _getSlideAnimation(index: number, toValue: number) {
        const { interpolators } = this.state;
        const { activeAnimationType, activeAnimationOptions={} } = this.props;

        const animatedValue = interpolators && (interpolators[index] as Animated.Value);
    ...
    private _playCustomSlideAnimation(current: number, next: number ) {
        const { interpolators } = this.state;
    ...
    private _hackActiveSlideAnimation(index: number, goTo: 'start' | 'end', force: boolean = false) {
    ...
    private _repositionScroll(index: number) {
        const { data, loopClonesPerSide } = this.props;
        const dataLength = data && data.length;
    ...
    private _scrollTo(offset: number, animated: boolean = true) {
        const { vertical } = this.props;
        const wrappedRef = this._getWrappedRef();
    ...
    private _onStartShouldSetResponderCapture(event: GestureResponderEvent, gestureState: PanResponderGestureState): boolean {
        const { onStartShouldSetResponderCapture } = this.props;

        if (isFunction(onStartShouldSetResponderCapture)) {
    ...
    private _onMomentumScrollEnd(event: NativeSyntheticEvent<NativeScrollEvent>) {
    ...
    private _onScrollEnd() {
        const { autoplayDelay = 1000, enableSnap = true } = this.props;
    ...
    private _onTouchRelease() {
        const { enableMomentum = false } = this.props;
    ...
    private _snapScroll(delta: number) {
    ...
    private _snapToItem(index: number, animated = true, fireCallback = true, initial = false, lockScroll = true) {
        const { enableMomentum = false, onSnapToItem, onBeforeSnapToItem } = this.props;
        const itemsLength = this._getCustomDataLength();
        const wrappedRef = this._getWrappedRef();
        ...
        if (index !== this._previousActiveItem) {
            ...
            if (fireCallback) {
                if (isFunction(onBeforeSnapToItem)) {
                    this._canFireBeforeCallback = true;
                }

                if (isFunction(onSnapToItem)) {
    ...
    private _onBeforeSnap(index: number) {
        ...
        isFunction(onBeforeSnapToItem) && onBeforeSnapToItem(index);
    }

    private _onSnap(index: number) {
        ...
        isFunction(onSnapToItem) && onSnapToItem(index);
    }

    public startAutoplay() {
        const { autoplayInterval = 3000, autoplayDelay = 1000 } = this.props;
        this._autoplay = true;
    ...
    public snapToItem(index: number, animated = true, fireCallback = true) {
        if (!index || index < 0) {
            index = 0;
        }
    ...
    public snapToNext(animated = true, fireCallback = true) {
        const itemsLength = this._getCustomDataLength();
    ...
    public snapToPrev(animated = true, fireCallback = true) {
        const itemsLength = this._getCustomDataLength();
    ...
    private _getSlideInterpolatedStyle(index: number, animatedValue: Animated.AnimatedInterpolation) {
        ...
        if (this._shouldUseShiftLayout()) {
            return shiftAnimatedStyles(animatedValue, this.props);
        }

        return defaultAnimatedStyles(animatedValue, this.props);
    }

    private _renderItem({ item, index }: { item: ItemT, index: number }) {
        const { interpolators } = this.state;
        const {
            hasParallaxImages,
            itemWidth,
        ...
        const animatedValue = interpolators && (interpolators[index] as Animated.AnimatedInterpolation );
    ...
    private _getComponentOverridableProps() {
        const {
