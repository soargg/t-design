...
class ToastView extends React.PureComponent<ToastPops, ToastState> {
...
class ToastConstructor {
    ...
    private createToast(toast: React.ReactNode, during: number = 3000) {
    ...
    private iconToast(txt: string, during: number, type: string) {
        ...
        const toast: React.ReactNode = (
            <View style={[styles.wrap, styles.iconToast]}>
                {
                    type === 'loading' ?
                    <AnimationView
                ...
                <Text
                    ellipsizeMode="tail"
                    numberOfLines={1}
                    style={[styles.font, styles.iconFont]}>{ txt }</Text>
            </View>
        );
    ...
    info(txt: string, during?: number): void {
        const toast: React.ReactNode = (
            <View style={[styles.wrap, styles.txtToast]}>
...
const txtToastWidth: number = screenWidth * 542 / 750;
